...
@WebFilter(filterName = "AuthUsuario", urlPatterns = Constantes.URL_USUARIO)
public class Usuario extends HttpFilter {
    ...
    protected void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if(method.equalsIgnoreCase("DELETE") || method.equalsIgnoreCase("PUT") ){
            if (!controlador.setUserAndPassword(request, response) || !controlador.verificarCredencial(response, com.trabajofinal.utils.servlets.autentificacion.Constantes.CREDENCIAL_SIMPLE))
                return;
        }
